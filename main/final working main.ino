...
uint8_t getFingerprintID() {
  ...
  while(magState == 1 && locktimer <= 30){//0 is closed, 1 open
    read_button(button1);
    if(buttonState == 1){
...
void loop() {
  ...
  if(validated){
    ...
    battery_time = battery_time + time/1000;
  }
  else{
    turn_on_LED(rr);
    delay(3000);
